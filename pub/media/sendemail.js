require(['jquery', 'jquery/jquery.rut','jquery/jquery.validate2'], function($, jrut, jqueryvalid){ 
  ...
  if ($('#leasing_operativo_tab .btn-primary').length > 0) {
    ...
    $('.form_right form .step_one .btn_action .btn-primary').click( function(e){
      e.preventDefault();
      $('.rut-err').remove();
      if ($.Rut.validar($('#leasing_operativo_tab [name="rut"]').val())) {

        $("#leasing_operativo_tab form").valid();
        if ($('.form_right form .step_one .input-group > label.error:visible').length < 1) {

          $(this).parent().parent().parent().parent().removeClass('step_one_active');
          $(this).parent().parent().parent().parent().addClass('step_two_active');
          $('.form_right .tab-content p.steps').html('2.Datos contacto <small>2/2</small>');
        };
      }
      else{
      ...
      if ($("#leasing_operativo_tab form").valid()) {
            ...
            $.ajax({
                  ...
                  success: function () {
                    ...
                    $('.form_right .tab-content p.steps').html('1.Datos Empresa  <small>1/2</small>');
